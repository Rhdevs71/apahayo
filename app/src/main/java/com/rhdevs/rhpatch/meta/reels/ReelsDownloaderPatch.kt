            ...
            XposedBridge.hookAllMethods(bottomSheetClass, "onViewCreated", object : XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam) {
                    try {
                        ...
                        // Guard: HANYA tampilkan tombol unduh Reels jika konteks aktif benar-benar Reels (clips_viewer)
                        // Mencegah muncul dobel di postingan Feed foto/slide carousel biasa
                        if (currentClipsHelper == null) return
