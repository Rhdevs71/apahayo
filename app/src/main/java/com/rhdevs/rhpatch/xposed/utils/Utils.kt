...
import com.rhdevs.rhpatch.xposed.core.WppCore.getClientBridge
...
object Utils {
    ...
    fun doRestart(context: Context): Boolean {
        try {
            ...
            try {
                // Try to force stop and restart via root if available (LSPosed Manager usually runs in root)
                val p = Runtime.getRuntime().exec(arrayOf("su", "-c", "am force-stop ${context.packageName} && am start -n ${componentName?.flattenToString()}"))
                if (p.waitFor() == 0) return true
            } catch (e: Exception) {
                // Ignore if no root
            }
            
            // Non-root fallback: standard clean restart
            context.startActivity(mainIntent)
            
            // Matikan proses secara rapi (bukan exit(0) yang bisa memicu ANR/Crash state)
