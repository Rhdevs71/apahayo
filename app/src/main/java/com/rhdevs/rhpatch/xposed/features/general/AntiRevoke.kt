...
import com.rhdevs.rhpatch.xposed.core.components.StatusItemWpp
import com.rhdevs.rhpatch.xposed.core.components.WaContactWpp
import com.rhdevs.rhpatch.xposed.core.db.DelMessageStore
...
class AntiRevoke(loader: ClassLoader, preferences:SharedPreferences) :
    ...
    override fun doHook() {
        ...
        XposedBridge.hookMethod(antiRevokeMessageMethod, object : XC_MethodHook() {
            override fun beforeHookedMethod(param: MethodHookParam) {
                ...
                val messageId = messageKey.messageID
                ...
                if (shouldIntercept) {
                    ...
                    if (method.returnType != Boolean::class.javaPrimitiveType) {
                        try {
                            val constructor = method.returnType.constructors.firstOrNull()
                            if (constructor != null) {
                ...
                if (messageId != null) {
                    if (date != null) {
                        ...
                        dateTextView.setOnClickListener {
                            ...
                            val toastMessage =
                                Utils.getString(R.string.message_removed_on)
                                    .format(date)
                    ...
                    when (antirevokeValue) {
                        ...
                        2 -> {
                            val drawable = Utils.getDrawable(R.drawable.deleted)
    ...
    private fun formatRevocationMessage(fMessage: FMessageWpp): String? {
        ...
        var messageSuffix = Utils.getString(R.string.deleted_message)

        if (jidAuthor.isStatus) {
            messageSuffix = Utils.getString(R.string.deleted_status)
        ...
        return if (jidAuthor.isGroup) {
            ...
            if (participantJid.isNull) {
                val deletedAdminUser = runCatching { XposedHelpers.getObjectField(fMessage.getObject(), "A00") }.getOrNull()
