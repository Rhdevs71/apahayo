    ...
    override fun run() {
        try {
            ...
            httpClient.newCall(request).execute().use { response ->
                ...
                val hash = if (tagName.contains("-")) tagName.split("-")[1].trim() else tagName.removePrefix("v").trim()
                val changelog = release.optString("body", "No changelog available.").trim()
                val publishedAt = release.optString("published_at", "")

                if (hash.isBlank()) return

                val currentVer = BuildConfig.VERSION_NAME
                val isNewVersion = !currentVer.lowercase().contains(hash.lowercase().trim()) && !hash.lowercase().contains(currentVer.substringBefore(" ").lowercase().trim())
                ...
                if (isNewVersion && !isIgnored) {
                    ...
                    mActivity.runOnUiThread {
                        val dialog = AlertDialogWpp(mActivity)
                        dialog.setTitle("Pembaruan")
                        dialog.setMessage("Versi RHPatch saat ini ($currentVer) sudah yang terbaru.")
                        dialog.setPositiveButton("OK") { d, _ -> d.dismiss() }
                        dialog.show()
                    }
                }
